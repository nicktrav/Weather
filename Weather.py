...
def forecast():
    ...
    for forecast in forecasts[::-1]:
        forecast_date = dateutil.parser.parse(
            forecast.attrib['start-time-local'])
        forecast_temp_max = getAttribute(
            forecast, 'air_temperature_maximum')
        forecast_temp_min = getAttribute(
            forecast, 'air_temperature_minimum')
...
def current():
    ...
    print '\nLatest observations for %s (as of %s local time):' % (
        location, time)
...
def main():
    ...
    parser.add_argument("-f", "--forecast",
                        help="7 day forecast", action="store_true")
    parser.add_argument("-c", "--current",
                        help="Current temperature", action="store_true")
    parser.add_argument("-p", "--past",
                        help="Temperatures frorm the last 24 hours",
                        action="store_true")
